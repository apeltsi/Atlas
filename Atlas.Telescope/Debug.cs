...
public static class Debug
{
    ...
    internal static Dictionary<string, TelescopeAction> actions = new();
    ...
    {
        var exists = Process.GetProcessesByName(Path.GetFileNameWithoutExtension(Assembly.GetEntryAssembly().Location))
            .Count() > 1;
        var disable = ArgsIncludeString("--disable-multi-process-debugging");
        if (exists || disable)
        {
            if (disable) Console.WriteLine("Multi Process Debugging is Disabled!");
        ...
        childProcess.OutputDataReceived += (sender, args) =>
        {
            if (args.Data != null)
            {
                ...
                {
                    if (args.Data.ToLower().Contains("error") || args.Data.ToLower().Contains("err") ||
                        args.Data.ToLower().Contains("exception"))
                        Console.ForegroundColor = ConsoleColor.Red;
        ...
        while (!childProcess.HasExited) Thread.Sleep(1000);
        FileLogs.Dispose();
        Environment.Exit(0);
    }
    ...
    public static void StartLogs(string[] categories)
    {
        ...
        catch (Exception e)
        {
            ...
            if (e.StackTrace != null)
                Error(0, e.StackTrace);
        }
#endif
        logsEnabled = true;
    }

    private static void PerformLog(string prefix, string log, string category = "")
    {
        if (category == "" || !Categories.Contains(category)) category = Categories[0];
        var logtext = GetTimestamp() + " " + prefix + " " + category + " > " + log;
    ...
    {
        var index = category.ToInt32(CultureInfo.CurrentCulture);
    ...
    private static DateTime GetBuildDateTime(Assembly assembly)
    {
        ...
        if (File.Exists(path))
        {
            ...
            try
            {
                var coffHeader = (_IMAGE_FILE_HEADER)Marshal.PtrToStructure(pinnedBuffer.AddrOfPinnedObject(),
                    typeof(_IMAGE_FILE_HEADER));

                return TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1) +
                                                            new TimeSpan(coffHeader.TimeDateStamp *
