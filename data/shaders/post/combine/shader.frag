...
layout(set = 0, binding = 2) uniform texture2D BloomColor;
...
void main()
{
    ...
    //fsout_Color=mix(texture(sampler2D(Color, Sampler), fsin_UV.xy), texture(sampler2D(BloomColor, Sampler), fsin_UV.xy), 0.5);
}