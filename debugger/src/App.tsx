...
import Debugger, { DebuggerData, Log, LogType } from './Debugger';
import { LiveData } from './LiveHeader';
import { addData } from './Profiler';
import Loading from './Loading';
export const [debuggerState, setDebuggerState] = createStore<DebuggerData>({ runDate: "N/A", engineVersion: "Unknown version", live: false } as DebuggerData);
export const [logState, setLogState] = createStore<Log[]>([]);
export const [subSystemsState, setSubSystemState] = createStore<string[]>([]);
export const [liveDataState, setLiveDataState] = createStore<LiveData>({ fps: 0, runtime: 0, connected: false } as LiveData);
...
const App: Component = () => {
  ...
  return (
    <div class={styles.App}>
      <Show when={debuggerState.live && !liveDataState.connected && liveDataState.runtime === 0}>
        <Loading></Loading>
      </Show>
      <Debugger></Debugger>
    </div>
...
function StartListening() {
  ...
  socket.onopen = () => {
    ...
    sendMessage = (data) => { socket.send(data) };
  ...
  socket.addEventListener('message', function (event) {
    ...
    if (data !== undefined && data.type !== undefined) {
      ...
      } else if (data.type === "livedata" && data.framerate !== undefined && data.runtime !== undefined) {
        setLiveDataState({ fps: data.framerate, runtime: data.runtime });
...
function parseData(): DebuggerData | null {
  let data = document.getElementById("logs")?.innerText;
  if (data === undefined || data === "") {
    console.log("Running in live mode");
    setDebuggerState({ live: true });
    return { runDate: "N/A", engineVersion: "Unknown version", live: true } as DebuggerData;
  ...
  return { runDate: lines[1].replace("RUN_DATE: ", ""), engineVersion: lines[0].replace("ENGINE_VERSION: ", ""), live: false } as DebuggerData;
}
...
function parseLine(log: string) {
  ...
  return { date: date, type: type, subSystem: subSystem, content: content } as Log;
}
