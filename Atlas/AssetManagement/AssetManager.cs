...
{
    private static ConcurrentDictionary<string, WeakReference<Asset>> loadedAssets = new();
    private static ConcurrentDictionary<string, Asset> keepAliveAssets = new();

    private static Dictionary<string, List<string>>
        assetMap = new(); // <string: path, string[] index 0 = assetpackname index n = truepath of file(s)
    ...
    internal static void LoadAssetMap()
    {
        try
        {
            var assetmapPath = Path.Join(Atlas.AssetPackDirectory, ".assetmap");
            var map = new Dictionary<string, List<string>>();
            if (File.Exists(assetmapPath))
            {
                var tempmap = JsonSerializer.Deserialize<Dictionary<string, string>>(File.ReadAllText(assetmapPath));
                foreach (var item in tempmap)
                {
                    var extensionlessPath =
                        item.Key.Substring(0, item.Key.Length - (item.Key.Split(".").Last().Length + 1));
            ...
            else
            {
                Debug.Log(LogCategory.Framework,
                    "Atlas starting without an AssetMap. Assets won't be loaded from AssetPacks unless manually loaded.");
                return;
            }
        ...
        {
            Debug.Error(LogCategory.Framework, "Couldn't load AssetMap: " + e);
        }
    }
    ...
    public static T? LoadAssetToMemory<T>(string path, AssetMode mode) where T : Asset, new()
    {
        ...
        {
            var paths = assetMap[prefix + path];
            Debug.Warning(LogCategory.Framework,
                "AssetPack containing asset '" + path + "' was not loaded. Attempting to load asset manually from '" +
                paths[0] + "'");

            var pack = new AssetPack(paths[0]);
        ...
        return FinalizeLoadingAsset(a, mode, path);
    }
    ...
    private static T? FinalizeLoadingAsset<T>(T a, AssetMode mode, string path) where T : Asset, new()
    {
        ...
        if (mode == AssetMode.KeepAlive) keepAliveAssets.TryAdd(path, a);
        return a;
    }
    ...
    public static void FreeAsset(string path)
    {
        if (keepAliveAssets.ContainsKey(path))
        {
            ...
            if (asset != null) keepAliveAssets.TryRemove(new KeyValuePair<string, Asset>(path, asset));
        }
    }
    ...
    /// <summary>
    /// WARNING: This method runs garbage collection. IT CAN BE VERY EXPENSIVE. Generally it is advised to let the garbage
    /// collector decide when the time is ripe.
    /// </summary>
    ...
    internal static void Dispose()
    {
        ...
        AssetPack.assetHandlers = new Dictionary<string, AssetPack.AssetHandler>();
        AssetPack.loadedAssetPacks = new Dictionary<string, AssetPack>();
        AssetPack.loadFiles = new Dictionary<string, List<string>>();
    }

    /// <summary>
    /// Removes some null values from loadedAssets. Note that this doesn't actually unload any assets, only cleanup the
    /// remaining nulls from unloaded assets.
    /// </summary>
    private static void Cleanup()
    {
        lock (loadedAssets) // Lets make sure that we have the assets for ourselves ;)
        {
            var curAssets = loadedAssets.ToDictionary(kvp => kvp.Key, kvp => kvp.Value, loadedAssets.Comparer);
            foreach (var asset in curAssets)
            {
                Asset? a;
                asset.Value.TryGetTarget(out a);
                if (a == null) loadedAssets.Remove(asset.Key, out _);
            }
        }
    }
}